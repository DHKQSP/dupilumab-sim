...
fd[, cell := sprintf("%s %s %s x%.3g", c(k2016 = "2016", k2020 = "M1")[pk_model], mechanism, direction, multiplier)]
...
pl <- ggplot(fd, aes(x = pass_pct, y = cell, colour = analysis_model)) +
  ...
  labs(x = "Boundary type I error, % (Wilson 95% interval)", y = NULL,
       title = "Boundary type I error by analysis model", subtitle = "16 boundary cells (true AUC0-inf ratio 0.80 or 1.25); dashed line 5%") +

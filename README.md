...
.github/workflows/            CI: tests.yml(fast 범위 테스트), repro.yml(재현성 점검: 수동 실행 또는 커밋 요청)
...
- **재현성 점검** (`.github/workflows/repro.yml`, 수동 실행 또는 커밋 요청): GitHub 저장소의 Actions 탭 > repro > Run workflow, 또는 `gh workflow run repro.yml --ref <브랜치>`. 수동 실행 권한이 없는 도구에서는 `config/repro_request.yaml`에 날짜·사유를 추가해 push하면 실행된다(D-050). 깨끗한 renv 복원 → fast 테스트 → `scripts/38_repro_check.R` 순서로 실행한다. 비교 항목은 (a) 커밋된 OC 시험(k2016 시험 1–100, S00·Vmax_down_125)의 정확 재생성, (b) Vmax 역산 배율의 참 AUC0-inf 비(공통 난수 앞 20,000명 대 200,000명), (c) B0 개인 참 외삽 비율 중앙값(2,000명 대 20,000명)이다. 허용 오차는 `config/repro_check.yaml`에 사전 고정했다. (a)의 기준은 `results/oc/oc_trials_be_k2016.csv.gz`이며, 이 파일이 저장소에 없으면 발췌본 `results/repro/reference/`(`--extract-reference`로 생성, 둘 다 있으면 일치 확인)을 쓴다. 결과 `results/repro/repro_check.csv`와 `repro_check_meta.csv`(커밋 해시, R·rxode2 버전, 플랫폼, 시각)는 artifact `repro-results`와 작업 요약에 남는다. 하나라도 불통과면 실행이 실패한다. 로컬 실행: `Rscript scripts/38_repro_check.R [코어]`. GitHub 실행 결과는 잡 로그 원문 발췌(`results/repro/repro_github_job_log_excerpt.txt`, API 메타데이터 포함)에서 `scripts/38b_repro_github_record.R`로 `repro_github_run.csv`·`repro_github_items.csv`를 만들어 보고서 부록 C와 `summary_en.md` 9절에 싣는다(D-051).
- **renv**: `renv.lock`은 스크립트가 쓰는 패키지를 모두 기록한다(`renv::status()` 일치). xml2는 `scripts/run_tests.R`의 선택적 JUnit 출력에만 쓰므로 renv 무시 패키지다(`renv/settings.json`, D-051).
